...
m1 <- fit_issf(Fanbelt_issa,
               ...
               case_ ~
                 ...
                 NormDistEdge_start:log_sl_ + NormDistEdge_start:cos_ta_+
...
write.csv(aic_table,
          row.names = FALSE,
          file = paste("/Users/rhemitoth/Documents/Lion_Movement/R/hip_lion_issa/Model_Competition_nde/",
                       lion_name,
                       "AIC.csv"))
index <- which.min(aic_table$AIC)
Fanbelt_winner <- aic_table[index,]
Fanbelt_model <- get(Fanbelt_winner$Model[[1]])
