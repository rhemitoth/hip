...
#Loading Data----
source("/Users/rhemitoth/Documents/Lion_Movement/R/hip_lion_issa/scripts/Manuscript/PreyAbundance.R")

#All Herbivore RSF----
#Initializing RMSE results tables----
cv_results <- tibble(Model = "",
                             fold = rep(seq(1,10,1),times = 7),
                             RMSE = 0.001,
                             R2 = 0.001,
                             AUC = 0.001)
...
for(i in 1:10){
  ...
  #Avg NDVI
  data.rsf.train$ndvi <- raster::extract(ndvi, data.rsf.train[,1:2])

  #Remove rows containing NA

  data.rsf.train <- na.omit(data.rsf.train)

  RSF1.fit.train <- glm(Used ~ veg1m + ndvi +slope + rivers,
                               data = data.rsf.train,
...
for(i in 1:10){
  ...
  RSF3.fit.train <- glm(Used ~ veg1m + ndvi +slope,
                               data = data.rsf.train,
                               family = binomial(link = "logit"))
...
for(i in 1:10){
  ...
  RSF4.fit.train <- glm(Used ~ veg1m + ndvi,
                               data = data.rsf.train,
                               family = binomial(link = "logit"))
  ...
  RSF4.predict.train <- terra::predict(object = hab1, model = RSF4.fit.train, type = "response")
...
for(i in 1:10){
  ...
  RSF5.fit.train <- glm(Used ~ veg1m,
                               data = data.rsf.train,
                               family = binomial(link = "logit"))

  hab1 <- stack(veg1m)
  names(hab1) <- c("veg1m")
  RSF5.predict.train <- terra::predict(object = hab1, model = RSF5.fit.train, type = "response")

  #*****Extracting RSF score for test data----

  data.rsf.test$RSF <- raster::extract(RSF5.predict.train, data.rsf.test[,1:2])
...
for(i in 1:10){
  ...
  data.rsf.train$ndviFire <- NA

  for ( i in 1:length(data.rsf.train$X)){
    date <- data.rsf.train$ndviDate[i]
    if (date == "2014-07-22"){
      data.rsf.train$ndviFire[i] <- raster::extract(ndvi_20140722, data.rsf.train[i,1:2])
    } else if (date == "2014-08-23"){
      data.rsf.train$ndviFire[i] <- raster::extract(ndvi_20140823, data.rsf.train[i,1:2])
    }
  }
  ...
  RSF9.fit.train <- glm(Used ~ veg1m + ndviFire+slope+rivers,
                               data = data.rsf.train,
                               family = binomial(link = "logit"))

  coef(RSF9.fit.train)%>%exp()

  vif(RSF9.fit.train)

  hab9_0722<- stack(ndvi_20140722, slope, veg1m,rivers)
  names(hab9_0722) <- c("ndviFire", "slope", "veg1m", "rivers")
  RSF9_0722.predict.train <- terra::predict(object = hab9_0722, model = RSF9.fit.train, type = "response")

  hab9_0823 <- stack(ndvi_20140823, slope, veg1m,rivers)
  names(hab9_0823) <- c("ndviFire", "slope", "veg1m", "rivers")
  RSF9_0823.predict.train <- terra::predict(object = hab9_0823, model = RSF9.fit.train, type = "response")

  #*****Extracting RSF score for test data----

  data.rsf.test$RSF <- NA

  for( i in 1:length(data.rsf.test$X)){
    if (data.rsf.test$ndviDate[i] == "2014-07-22"){
      data.rsf.test$RSF[i] <- raster::extract(RSF9_0722.predict.train, data.rsf.test[i,1:2])
    } else if(data.rsf.test$ndviDate[i] == "2014-08-23"){
      data.rsf.test$RSF[i] <- raster::extract(RSF9_0823.predict.train, data.rsf.test[i,1:2])
    }
  }
  ...
  sst <- sum((obs_data - obs_mean)^2)
  r2 <- 1 - (sse/sst)
  cv_results$R2[f+40] <- r2

  #*****RMSE----
  rmse <- sqrt(mean((as.numeric(data.rsf.test$Used)-data.rsf.test$RSF)^2 ))

  cv_results$RMSE[f+40] <- rmse
...
for(i in 1:10){
  ...
  RSF10.fit.train <- glm(Used ~ veg1m + ndviFire+ slope,
                                data = data.rsf.train,
                                family = binomial(link = "logit"))

  coef(RSF10.fit.train)%>%exp()

  vif(RSF10.fit.train)

  hab9_0722<- stack(ndvi_20140722, slope, veg1m)
  names(hab9_0722) <- c("ndviFire", "slope", "veg1m")
  RSF10_0722.predict.train <- terra::predict(object = hab9_0722, model = RSF10.fit.train, type = "response")

  hab9_0823 <- stack(ndvi_20140823, slope, veg1m)
  names(hab9_0823) <- c("ndviFire", "slope", "veg1m")
  RSF10_0823.predict.train <- terra::predict(object = hab9_0823, model = RSF10.fit.train, type = "response")
  ...
  for( i in 1:length(data.rsf.test$X)){
    if (data.rsf.test$ndviDate[i] == "2014-07-22"){
      data.rsf.test$RSF[i] <- raster::extract(RSF10_0722.predict.train, data.rsf.test[i,1:2])
    } else if(data.rsf.test$ndviDate[i] == "2014-08-23"){
      data.rsf.test$RSF[i] <- raster::extract(RSF10_0823.predict.train, data.rsf.test[i,1:2])
    }
  }
  ...
  sst <- sum((obs_data - obs_mean)^2)
  r2 <- 1 - (sse/sst)
  cv_results$R2[f+50] <- r2

  #*****RMSE----
  rmse <- sqrt(mean((as.numeric(data.rsf.test$Used)-data.rsf.test$RSF)^2 ))

  cv_results$RMSE[f+50] <- rmse
...
for(i in 1:10){
  ...
  RSF11.fit.train <- glm(Used ~ veg1m + ndviFire,
                                data = data.rsf.train,
                                family = binomial(link = "logit"))

  coef(RSF11.fit.train)%>%exp()

  vif(RSF11.fit.train)

  hab9_0722<- stack(ndvi_20140722, veg1m)
  names(hab9_0722) <- c("ndviFire", "veg1m")
  RSF11_0722.predict.train <- terra::predict(object = hab9_0722, model = RSF11.fit.train, type = "response")

  hab9_0823 <- stack(ndvi_20140823, veg1m)
  names(hab9_0823) <- c("ndviFire", "veg1m")
  RSF11_0823.predict.train <- terra::predict(object = hab9_0823, model = RSF11.fit.train, type = "response")
  ...
  for( i in 1:length(data.rsf.test$X)){
    if (data.rsf.test$ndviDate[i] == "2014-07-22"){
      data.rsf.test$RSF[i] <- raster::extract(RSF11_0722.predict.train, data.rsf.test[i,1:2])
    } else if(data.rsf.test$ndviDate[i] == "2014-08-23"){
      data.rsf.test$RSF[i] <- raster::extract(RSF11_0823.predict.train, data.rsf.test[i,1:2])
    }
  }
  ...
  sst <- sum((obs_data - obs_mean)^2)
  r2 <- 1 - (sse/sst)
  cv_results$R2[f+60] <- r2

  #*****RMSE----
  rmse <- sqrt(mean((as.numeric(data.rsf.test$Used)-data.rsf.test$RSF)^2 ))

  cv_results$RMSE[f+60] <- rmse
...
write.csv(cv_sum,
          "/Users/rhemitoth/Documents/Lion_Movement/R/hip_lion_issa/exports/cv_results/Herbivore_RSF_Model_Selection.csv")

write.csv(cv_sum,
          "/Users/rhemitoth/Documents/Lion_Movement/Manuscript/PreyAbundance/PreyAbundance_RSF_CV.csv")
...
roc_plot <- ggplot(rocs, aes(x = fpr, y = tpr))+
  ...
  ggtitle("Model ROC Curves (All Herbivores)")+
  scale_color_discrete(name = "Model (AUC)")


roc_plot

ggsave(filename = "/Users/rhemitoth/Documents/Lion_Movement/R/hip_lion_issa/exports/plots/ROC/ROC.png",
       plot = roc_plot,
       units = c("in"),
       width = 6,
       height = 5,
