## ****************************************************
## Title: Fourth Order Selection RSF Cross Validation
## Author: Rhemi Toth
...
#Loading Data----
source("~/Documents/Lion_Movement/R/hip_lion_issa/scripts/fourth_order_RSF/Fourth_Order_RSF.R")

#Makign Kill a factor
data.rsf$Kill <- factor(data.rsf$Kill, levels = c(0,1))
#All Herbivore RSF----
...
for(i in 1:10){
  ...
  data.rsf.train$PreyCatchability <- raster::extract(pc_map, data.rsf.train[,1:2])

  # Prey Abundance and NDVI

  data.rsf.train$ndvi <- 0.001
  data.rsf.train$PreyAbundance <- 0.001
  for(i in 1:nrow(data.rsf.train)){
    date <- data.rsf.train$ndviDate[i]
    index <- which(ndvi_dates == date)
    ndviImg <- get(ndvi_images[index])
    pa_map <- get(pa_maps[index])
    data.rsf.train$ndvi[i] <- raster::extract(ndviImg, data.rsf.train[i,1:2])
    data.rsf.train$PreyAbundance[i]  <- raster::extract(pa_map, data.rsf.train[i,1:2])
  }
  ...
  RSF1.fit.train <- glm(Kill ~ PreyAbundance,
                        data = data.rsf.train,
                        family = binomial(link = "logit"))

  coef(RSF1.fit.train)%>%exp()

  hab1_0607<- stack(pa_20150607)
  names(hab1_0607) <- c("PreyAbundance")
  RSF1_0607.predict.train <- terra::predict(object = hab1_0607, model = RSF1.fit.train, type = "response")

  hab1_0623 <- stack(pa_20150623)
  names(hab1_0623) <- c("PreyAbundance")
  RSF1_0623.predict.train <- terra::predict(object = hab1_0623, model = RSF1.fit.train, type = "response")

  hab1_0709 <- stack(pa_20150709)
  names(hab1_0709) <- c("PreyAbundance")
  RSF1_0709.predict.train <- terra::predict(object = hab1_0709, model = RSF1.fit.train, type = "response")

  hab1_0927 <- stack(pa_20150927)
  names(hab1_0927) <- c("PreyAbundance")
  RSF1_0927.predict.train <- terra::predict(object = hab1_0927, model = RSF1.fit.train, type = "response")

  maps <- c('RSF1_0607.predict.train','RSF1_0623.predict.train','RSF1_0709.predict.train','RSF1_0927.predict.train')

  #*****Extracting RSF score for test data----

  data.rsf.test$RSF <- 0.001
  for(i in 1:nrow(data.rsf.test)){
    date <- data.rsf.test$ndviDate[i]
    index <- which(ndvi_dates == date)
    map <- get(pa_maps[index])
    data.rsf.test$RSF[i] <- raster::extract(map, data.rsf.test[i,1:2])
  }
  #Remove rows containing NA
  ...
  #*****R2----
  obs_data <- as.numeric(data.rsf.test$Kill)
  ...
  #*****RMSE----
  rmse <- sqrt(mean((as.numeric(data.rsf.test$Kill)-data.rsf.test$RSF)^2 ))

  cv_results$RMSE[f] <- rmse

  #*****ROC----
  pred <- prediction(data.rsf.test$RSF, data.rsf.test$Kill)
...
for(i in 1:10){
  ...
  RSF2.fit.train <- glm(Kill ~ PreyCatchability,
                        data = data.rsf.train,
                        family = binomial(link = "logit"))

  coef(RSF2.fit.train)%>%exp()

  hab1 <- stack(pc_map)
  names(hab1) <- c("PreyCatchability")
...
for(i in 1:10){
  ...
  RSF3.fit.train <- glm(Kill ~ slope + veg1m + ndvi,
                        data = data.rsf.train,
                        family = binomial(link = "logit"))

  hab1_0607<- stack(ndvi_20150607, slope, veg1m)
  names(hab1_0607) <- c("ndvi", "slope", "veg1m")
  RSF3_0607.predict.train <- terra::predict(object = hab1_0607, model = RSF3.fit.train, type = "response")

  hab1_0623 <- stack(ndvi_20150623, slope, veg1m)
  names(hab1_0623) <- c("ndvi", "slope", "veg1m")
  RSF3_0623.predict.train <- terra::predict(object = hab1_0623, model = RSF3.fit.train, type = "response")

  hab1_0709 <- stack(ndvi_20150709, slope, veg1m)
  names(hab1_0709) <- c("ndvi", "slope", "veg1m")
  RSF3_0709.predict.train <- terra::predict(object = hab1_0709, model = RSF3.fit.train, type = "response")

  hab1_0927 <- stack(ndvi_20150927, slope, veg1m)
  names(hab1_0927) <- c("ndvi", "slope", "veg1m")
  RSF3_0927.predict.train <- terra::predict(object = hab1_0927, model = RSF3.fit.train, type = "response")

  maps <- c('RSF3_0607.predict.train','RSF3_0623.predict.train','RSF3_0709.predict.train','RSF3_0927.predict.train')
...
for(i in 1:10){
  ...
  RSF4.fit.train <- glm(Kill ~ veg1m +slope,
                        data = data.rsf.train,
                        family = binomial(link = "logit"))
...
write.csv(cv_sum,
          "~/Documents/Lion_Movement/R/hip_lion_issa/scripts/fourth_order_RSF/Kill_Site_RSF_Cross_Validation.csv" )
...
roc_plot <- ggplot(rocs, aes(x = fpr, y = tpr))+
  ...
  ggtitle("Kill Site RSF ROC Curves")+
  scale_color_discrete(name = "Model (AUC)")


roc_plot

ggsave(filename = "~/Documents/Lion_Movement/R/hip_lion_issa/scripts/fourth_order_RSF/Kill_Site_RSF_ROC.png",
       plot = roc_plot,
       units = c("in"),
       width = 6,
       height = 5,
